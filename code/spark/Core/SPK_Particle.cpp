...
namespace SPK
{
	...
	void Particle::init()
	{
		...
		for (size_t i = model->nbEnableParams; i != 0; --i)
		{
			...
			if (model->isInterpolated(param))
			{
				...
				float ratioY = random(0.0f,1.0f);
				float offsetX = random(-offsetVariation,offsetVariation);
				float scaleX = 1.0f + random(-scaleVariation,scaleVariation);

				*particleInterpolatedIt++ = ratioY;
				*particleInterpolatedIt++ = offsetX;
				*particleInterpolatedIt++ = scaleX;

				if (interpolator->getGraph().size() == 1)
				{
					// HACK constant interpolators get a random initial value
					*particleCurrentIt++ = model->interpolators[param]->interpolate(*this, param, ratioY, offsetX, scaleX);
				}
				else *particleCurrentIt++ = Model::DEFAULT_VALUES[param];
			}
	...
	void Particle::interpolateParameters()
	{
		...
		for (size_t i = 0; i < model->nbInterpolatedParams; ++i)
		{
			...
			if (model->isInterpolatorEnabled((SPK::ModelParam)index)) // HACK - don't run disabled interpolators in party
			{
