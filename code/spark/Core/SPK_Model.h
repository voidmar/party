...
namespace SPK
{
	...
	*/
	enum ModelParamFlag
	{
		...
		FLAG_SIZE_MULTIPLIER = 1 << PARAM_SIZE_MULTIPLIER,	/**< the flag bit for PARAM_SIZE_MULTIPLIER */
	...
	*/
	...
	public :
		...
		*/
		...
		int isInterpolatorEnabled(ModelParam type) const;
		void setInterpolatorEnabled(ModelParam type, bool value); // HACK turn off/on an enabled bit
	...
	inline int Model::isInterpolatorEnabled(ModelParam type) const
	{
		return interpolatorEnableFlag & (1 << type);
	}

	inline void Model::setInterpolatorEnabled(ModelParam type, bool value)
	{
		if (value) interpolatorEnableFlag = interpolatorEnableFlag | (1 << type);
		else interpolatorEnableFlag = interpolatorEnableFlag & ~(1 << type);
	}
